...
def main(file_path):  
    file_info = FileAccessInfo(file_path)  
    
    if file_info.get_file_stats():  
        last_access_time = file_info.get_last_access_time() 
        owner_sid = file_info.get_owner_sid()
        owner_sid_string = file_info.get_owner_sid_string()
        
        print(f"Last Access Time: {last_access_time}")  
        
        if owner_sid is not None:  
            print(f"File Owner SID: {owner_sid_string}")  
            username_info = FileAccessInfo.sid_to_username(owner_sid)  
            print(username_info)  


if __name__ == "__main__":  
    main(str(input(f"Path to file : ")))
